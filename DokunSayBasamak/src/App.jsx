  ...
  return (
    <div lang="tr" style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden", fontFamily: fontStyle, fontSize: baseFontSize, filter: highContrast ? "contrast(1.4) brightness(1.05)" : "none" }}>
        ...
        <nav aria-label="Araçlar" style={{ width: sidebarCollapsed ? 48 : 252, minWidth: sidebarCollapsed ? 48 : 252, background: `linear-gradient(180deg,${PALETTE.side},#efe8d6)`, borderRight: "1px solid " + PALETTE.sideB, display: "flex", flexDirection: "column", transition: "width .25s", overflow: "hidden" }}>
          {sidebarCollapsed ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "12px 0" }}>
              {[["📦", "mat", "ariaMenuBlocks"], ["📋", "act", "ariaMenuActivity"], ["🎮", "game", "ariaMenuGame"], ["⚙️", "feat", "ariaMenuSettings"]].map(([ic, id, ak]) => (
                <button key={id} onClick={() => { setSidebarCollapsed(false); setSidebarTab(id); }} aria-label={t(ak)} title={t(ak)} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid " + PALETTE.sideB, background: "#fff", cursor: "pointer", fontSize: 18 }}><span aria-hidden="true">{ic}</span></button>
              ))}
              <button onClick={() => setSidebarCollapsed(false)} aria-label={t("ariaExpand")} title={t("ariaExpand")} aria-expanded="false" style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid " + PALETTE.sideB, background: "#fff", cursor: "pointer", fontSize: 12, color: "#bbb" }}><span aria-hidden="true">▶</span></button>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
              <div style={{ padding: "8px 12px", display: "flex", alignItems: "center", gap: 8, borderBottom: "1px solid rgba(60,50,30,.06)" }}>
                ...
                <button onClick={() => setSidebarCollapsed(true)} aria-label={t("ariaCollapse")} title={t("ariaCollapse")} aria-expanded="true" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#ccc", width: 26, height: 26, borderRadius: 6 }}><span aria-hidden="true">◀</span></button>
              </div>
              <div role="tablist" style={{ display: "flex", padding: "4px 8px", gap: 3, background: "rgba(60,50,30,.02)" }}>
                {[["📦", "mat", t("tabBlocks")], ["📋", "act", t("tabAct")], ["🎮", "game", t("tabGame")], ["⚙️", "feat", t("tabSettings")]].map(([ic, id, lbl]) => (
                  <button key={id} role="tab" aria-selected={sidebarTab === id} aria-label={lbl} onClick={() => setSidebarTab(id)} style={{ flex: 1, padding: "6px 0", border: "none", borderRadius: 8, background: sidebarTab === id ? "#fff" : "transparent", cursor: "pointer", fontSize: 12, fontWeight: 800, color: sidebarTab === id ? PALETTE.text : "#bbb", fontFamily: "inherit", boxShadow: sidebarTab === id ? "0 1px 4px " + PALETTE.sh : "none" }} title={lbl}><span aria-hidden="true">{ic}</span></button>
                ))}
              </div>
        ...
        <main ref={mainAreaRef} aria-label="Blok çalışma alanı" style={{ flex: 1, position: "relative", overflow: "hidden" }}>
          <div ref={canvasRef} onClick={() => setSelectedBlock(null)} style={{ position: "absolute", inset: 0, overflow: "hidden", ...bgStyle }}>
            ...
            <div role="toolbar" aria-label="Çizim araçları" style={{ position: "absolute", top: 8, left: "50%", transform: "translateX(-50%)", zIndex: 30, display: "flex", gap: 1, background: "rgba(255,255,255,.94)", backdropFilter: "blur(16px)", borderRadius: 10, padding: "3px 5px", boxShadow: "0 2px 12px rgba(0,0,0,.05)", border: "1px solid rgba(0,0,0,.04)", alignItems: "center" }}>
              ...
              <button onClick={() => dispatch({ type: "UNDO_STROKE" })} disabled={!strokes.length} aria-label={t("undo")} title={t("undo")} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: strokes.length ? "pointer" : "default", fontSize: 13, color: strokes.length ? "#666" : "#ddd" }}><span aria-hidden="true">↩</span></button>
              <button onClick={() => dispatch({ type: "REDO_STROKE" })} disabled={!undone.length} aria-label={t("redo")} title={t("redo")} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: undone.length ? "pointer" : "default", fontSize: 13, color: undone.length ? "#666" : "#ddd" }}><span aria-hidden="true">↪</span></button>
              <div aria-hidden style={{ width: 1, height: 16, background: "rgba(0,0,0,.08)", margin: "0 1px" }} />
              <button onClick={() => dispatch({ type: "UNDO" })} disabled={!itemHistory.length} aria-label={t("ariaItemUndo")} title={t("ariaItemUndo")} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: itemHistory.length ? "pointer" : "default", fontSize: 11, color: itemHistory.length ? PALETTE.accent : "#ddd", fontWeight: 900 }}><span aria-hidden="true">⊘</span></button>
              <button onClick={() => dispatch({ type: "REDO" })} disabled={!itemFuture.length} aria-label={t("ariaItemRedo")} title={t("ariaItemRedo")} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: itemFuture.length ? "pointer" : "default", fontSize: 11, color: itemFuture.length ? PALETTE.accent : "#ddd", fontWeight: 900 }}><span aria-hidden="true">⊛</span></button>
              <div aria-hidden style={{ width: 1, height: 16, background: "rgba(0,0,0,.08)", margin: "0 1px" }} />
              <button onClick={() => { if (items.length > 0 || strokes.length > 0) { dispatch({ type: "CLEAR_ALL" }); } }} disabled={items.length === 0 && strokes.length === 0} aria-label={t("clear")} title={t("clear")} style={{ width: 28, height: 28, borderRadius: 6, border: "none", background: items.length > 0 || strokes.length > 0 ? "rgba(239,68,68,.08)" : "transparent", cursor: items.length > 0 || strokes.length > 0 ? "pointer" : "default", fontSize: 14, color: items.length > 0 || strokes.length > 0 ? "#ef4444" : "#ddd", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">🗑</span></button>
            </div>
                ...
                <div ref={matRef} data-mat="1" style={matXY ? { position: "absolute", left: matXY.x, top: matXY.y, zIndex: 4, transform: `scale(${matScale})`, transformOrigin: "top left" } : { position: "absolute", top: "50%", left: "50%", zIndex: 4, transform: `translate(-50%,-50%) scale(${matScale})`, transformOrigin: "center center", animation: "slideDown .3s" }}>
                  <div style={{ background: "rgba(255,255,255,.97)", backdropFilter: "blur(14px)", borderRadius: 16, overflow: "hidden", border: "1.5px solid rgba(60,50,30,.06)", boxShadow: "0 8px 32px " + PALETTE.shM }}>
                    <div onPointerDown={e => { e.preventDefault(); e.stopPropagation(); if (!canvasRef.current) return; const r = canvasRef.current.getBoundingClientRect(); const el = e.currentTarget.closest("[data-mat]"); if (!el) return; const er = el.getBoundingClientRect(); const ox = (er.left - r.left) / zoom, oy = (er.top - r.top) / zoom; const mx = (e.clientX - r.left) / zoom, my = (e.clientY - r.top) / zoom; setMatDrag({ offX: mx - ox, offY: my - oy }); if (!matXY) setMatXY({ x: ox, y: oy }); }}
                      style={{ background: `linear-gradient(135deg,${PALETTE.accent},${PALETTE.accentD})`, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, cursor: "grab", touchAction: "none", userSelect: "none" }}>
                      <button onClick={e => { e.stopPropagation(); setMatCols(c => Math.max(2, c - 1)); }} aria-label={t("ariaColLess")} title={t("ariaColLess")} style={{ width: 30, height: 30, borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.15)", cursor: "pointer", color: "#fff", fontSize: 18, fontWeight: 900 }}><span aria-hidden="true">−</span></button>
                      <span style={{ flex: 1, textAlign: "center", fontSize: 14, fontWeight: 900, color: "#fff", letterSpacing: 1.5 }}>{t("placeTable")}</span>
                      <button onClick={e => { e.stopPropagation(); setMatCols(c => Math.min(4, c + 1)); }} aria-label={t("ariaColMore")} title={t("ariaColMore")} style={{ width: 30, height: 30, borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.15)", cursor: "pointer", color: "#fff", fontSize: 18, fontWeight: 900 }}><span aria-hidden="true">+</span></button>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: `repeat(${visCols.length},minmax(96px,1fr))` }}>
                      {visCols.map(col => <div key={col.t + "h"} style={{ textAlign: "center", padding: "10px 4px", background: col.c + "10", borderBottom: "2px solid " + col.c + "30", borderRight: "1px solid rgba(60,50,30,.04)" }}><div style={{ fontSize: 16, fontWeight: 800, color: col.c }}>{col.l}</div><div style={{ fontSize: 12, color: col.c + "88" }}>{t("colSub")}</div></div>)}
                      {visCols.map(col => <button key={col.t + "+"} onClick={() => setMatCounts(p => ({ ...p, [col.t]: p[col.t] + 1 }))} aria-label={`${t(col.t === "ones" ? "colOnes" : col.t === "tens" ? "colTens" : col.t === "huns" ? "colHuns" : "colThs")} ${t("ariaPlaceMore")}`} title={t("ariaPlaceMore")} style={{ padding: "10px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,.04)", borderRight: "1px solid rgba(60,50,30,.03)", background: col.c + "08", cursor: "pointer", fontSize: 22, fontWeight: 900, color: col.c, fontFamily: "inherit", textAlign: "center" }}><span aria-hidden="true">+</span></button>)}
                      {visCols.map(col => { const v = matCounts[col.t]; return (<div key={col.t + "b"} style={{ height: 180, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 3, flexWrap: "wrap", padding: "6px", borderRight: "1px solid rgba(60,50,30,.03)", background: "rgba(255,255,255,.4)" }}>{v > 0 && Array.from({ length: Math.min(v, 9) }, (_, i) => <BlockSVG key={i} type={col.t} size={col.t === "ones" ? 1.8 : 0.33} colorBlind={colorBlind} />)}{v === 0 && <span style={{ fontSize: 28, color: "#e8dfd0" }}>—</span>}</div>); })}
                      {visCols.map(col => { const v = matCounts[col.t]; return <button key={col.t + "-"} onClick={() => setMatCounts(p => ({ ...p, [col.t]: Math.max(0, p[col.t] - 1) }))} disabled={v < 1} aria-label={`${t(col.t === "ones" ? "colOnes" : col.t === "tens" ? "colTens" : col.t === "huns" ? "colHuns" : "colThs")} ${t("ariaPlaceLess")}`} title={t("ariaPlaceLess")} style={{ padding: "10px 0", border: "none", borderTop: "1px solid rgba(0,0,0,.04)", borderBottom: "1px solid rgba(0,0,0,.06)", borderRight: "1px solid rgba(60,50,30,.03)", background: v > 0 ? col.c + "08" : "transparent", cursor: v > 0 ? "pointer" : "default", fontSize: 22, fontWeight: 900, color: v > 0 ? col.c : "#ddd", fontFamily: "inherit", textAlign: "center" }}><span aria-hidden="true">−</span></button>; })}
                      {visCols.map(col => <div key={col.t + "n"} style={{ textAlign: "center", padding: "8px 0", fontSize: 34, fontWeight: 900, color: col.c, borderRight: "1px solid rgba(60,50,30,.03)" }}>{matCounts[col.t]}</div>)}
                    </div>
                    {(() => {
                      const mtv = matCounts.ones + matCounts.tens * 10 + matCounts.huns * 100 + matCounts.ths * 1000;
                      const mOk = mtv > 0 ? readNum(mtv) : "";
                      return (
                        <div style={{ borderTop: "2px solid rgba(60,50,30,.06)", padding: "14px 18px", background: "rgba(60,50,30,.01)" }}>
                          <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 6 }}>
                            <span style={{ fontSize: 14, fontWeight: 700, color: "#b5a990", minWidth: 74 }}>{t("total")}</span>
                            <span style={{ fontSize: 36, fontWeight: 900, color: PALETTE.accentD }}>{mtv || "—"}</span>
                          </div>
                          {mOk && <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}><span style={{ fontSize: 14, fontWeight: 700, color: "#b5a990", minWidth: 74 }}>{t("reading")}</span><span style={{ fontSize: 18, fontWeight: 900, color: "#6d28d9", fontStyle: "italic" }}>{mOk}</span></div>}
                        </div>
                      );
                    })()}
                  </div>
      ...
      <div style={{ height: 46, minHeight: 46, background: `linear-gradient(180deg,${PALETTE.side},#ede5d0)`, borderTop: "1px solid rgba(60,50,30,.07)", display: "flex", alignItems: "center", padding: "0 10px", gap: 4 }}>
        <div role="group" aria-label={t("ariaBgNone") + " / " + t("ariaBgGrid") + " / " + t("ariaBgDot")} style={{ display: "flex", gap: 1, background: "rgba(255,255,255,.55)", borderRadius: 7, padding: 2 }}>
          {[["▫", "plain", "ariaBgNone"], ["▦", "grid", "ariaBgGrid"], ["⋯", "dot", "ariaBgDot"]].map(([ic, tp, ak]) => (
            <button key={tp} onClick={() => setBgType(tp)} aria-label={t(ak)} aria-pressed={bgType === tp} title={t(ak)} style={{ width: 26, height: 26, borderRadius: 5, border: "none", background: bgType === tp ? PALETTE.accent : "transparent", cursor: "pointer", fontSize: 11, color: bgType === tp ? "#fff" : "#aaa", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">{ic}</span></button>
          ))}
        </div>
        <div aria-hidden style={{ width: 1, height: 18, background: "rgba(60,50,30,.08)" }} />
        <div role="group" aria-label={t("ariaZoomIn") + " / " + t("ariaZoomOut")} style={{ display: "flex", gap: 1, alignItems: "center", background: "rgba(255,255,255,.55)", borderRadius: 7, padding: "2px 4px" }}>
          <button onClick={() => setZoom(z => Math.max(.5, +(z - .1).toFixed(1)))} aria-label={t("ariaZoomOut")} title={t("ariaZoomOut")} style={{ width: 22, height: 22, border: "none", background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 900, color: "#888" }}><span aria-hidden="true">−</span></button>
          <span style={{ fontSize: 10, fontWeight: 700, color: PALETTE.accentD, minWidth: 32, textAlign: "center" }} aria-live="polite" data-numeric="true">{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(z => Math.min(2, +(z + .1).toFixed(1)))} aria-label={t("ariaZoomIn")} title={t("ariaZoomIn")} style={{ width: 22, height: 22, border: "none", background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 900, color: "#888" }}><span aria-hidden="true">+</span></button>
        </div>
        <button onClick={toggleFullscreen} aria-label={isFullscreen ? "Tam ekrandan çık" : "Tam ekran"} aria-pressed={isFullscreen} title={isFullscreen ? "Tam ekrandan çık" : "Tam ekran"} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 13, background: isFullscreen ? PALETTE.accentL : "rgba(255,255,255,.55)", color: isFullscreen ? PALETTE.accentD : "#888", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">{isFullscreen ? "⊡" : "⊞"}</span></button>
        <div aria-hidden style={{ width: 1, height: 18, background: "rgba(60,50,30,.08)" }} />
        {SPEECH_SUPPORTED && <button onClick={toggleVoice} aria-label={voiceActive ? t("ariaVoiceStop") : t("ariaVoiceStart")} aria-pressed={voiceActive} title={voiceActive ? t("ariaVoiceStop") : t("ariaVoiceStart")} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 14, background: voiceActive ? "rgba(239,68,68,.12)" : "rgba(255,255,255,.55)", color: voiceActive ? "#ef4444" : "#888", display: "flex", alignItems: "center", justifyContent: "center", animation: voiceActive ? "micPulse 1.5s infinite" : "none" }}><span aria-hidden="true">{voiceActive ? "🔴" : "🎙️"}</span></button>}
        <button onClick={() => setShowRecPanel(p => !p)} aria-label={recording ? t("ariaRecordStop") : t("ariaRecord")} aria-pressed={recording} title={recording ? t("ariaRecordStop") : t("ariaRecord")} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 13, background: recording ? "rgba(239,68,68,.12)" : "rgba(255,255,255,.55)", color: recording ? "#ef4444" : "#888", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">{recording ? "⏹" : "⏺"}</span></button>
        <button onClick={() => setShowTeacher(true)} aria-label={t("ariaStats")} title={t("ariaStats")} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 13, background: "rgba(255,255,255,.55)", color: "#888", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">📊</span></button>
        <button onClick={() => setShowHelp(true)} aria-label={t("ariaHelp")} title={t("ariaHelp")} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 12, background: "rgba(255,255,255,.55)", color: "#888", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">?</span></button>
        <button onClick={() => setShowAbout(true)} aria-label={t("ariaInfo")} title={t("ariaInfo")} style={{ width: 28, height: 28, borderRadius: 7, border: "none", cursor: "pointer", fontSize: 12, background: "rgba(255,255,255,.55)", color: "#888", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">ℹ</span></button>
        <div style={{ flex: 1 }} />
        {(() => {
          const curIdx = pages.findIndex(p => p.id === currentPageId);
          const prevPg = pages[curIdx - 1]; const nextPg = pages[curIdx + 1];
          return (
            <nav role="navigation" aria-label={t("page")} style={{ display: "flex", alignItems: "center", gap: 2, background: "rgba(255,255,255,.6)", borderRadius: 8, padding: "2px", border: "1px solid rgba(60,50,30,.06)" }}>
              <button onClick={() => prevPg && dispatch({ type: "SWITCH_PAGE", pid: prevPg.id })} disabled={!prevPg} aria-label={t("ariaPrev")} title={t("ariaPrev")} style={{ width: 26, height: 26, borderRadius: 7, border: "none", cursor: prevPg ? "pointer" : "default", background: "transparent", fontSize: 14, color: prevPg ? "#888" : "#ddd", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">‹</span></button>
              <div style={{ minWidth: 44, textAlign: "center", fontSize: 11, fontWeight: 800, color: PALETTE.accentD, padding: "2px 4px", background: PALETTE.accentL, borderRadius: 5 }} aria-live="polite" aria-atomic="true"><span data-numeric="true">{curIdx + 1} / {pages.length}</span></div>
              <button onClick={() => nextPg && dispatch({ type: "SWITCH_PAGE", pid: nextPg.id })} disabled={!nextPg} aria-label={t("ariaNext")} title={t("ariaNext")} style={{ width: 26, height: 26, borderRadius: 7, border: "none", cursor: nextPg ? "pointer" : "default", background: "transparent", fontSize: 14, color: nextPg ? "#888" : "#ddd", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">›</span></button>
              <button onClick={() => dispatch({ type: "ADD_PAGE", id: Math.max(...pages.map(p => p.id)) + 1 })} aria-label={t("addPage")} title={t("addPage")} style={{ width: 26, height: 26, borderRadius: 7, border: "none", cursor: "pointer", background: "transparent", fontSize: 15, fontWeight: 700, color: "#aaa", display: "flex", alignItems: "center", justifyContent: "center" }}><span aria-hidden="true">+</span></button>
            </nav>
          );
        })()}
      </div>
