                ...
                <div ref={matRef} data-mat="1" style={matXY ? { position: "absolute", left: matXY.x, top: matXY.y, zIndex: 4, transform: `scale(${matScale})`, transformOrigin: "top left" } : { position: "absolute", top: "50%", left: "50%", zIndex: 4, transform: `translate(-50%,-50%) scale(${matScale})`, transformOrigin: "center center", animation: "slideDown .3s" }}>
                  <div style={{ background: "rgba(255,255,255,.97)", backdropFilter: "blur(14px)", borderRadius: 16, overflow: "hidden", border: "1.5px solid rgba(60,50,30,.06)", boxShadow: "0 8px 32px " + PALETTE.shM }}>
                    <div onPointerDown={e => { e.preventDefault(); e.stopPropagation(); if (!canvasRef.current) return; const r = canvasRef.current.getBoundingClientRect(); const el = e.currentTarget.closest("[data-mat]"); if (!el) return; const er = el.getBoundingClientRect(); const ox = (er.left - r.left) / zoom, oy = (er.top - r.top) / zoom; const mx = (e.clientX - r.left) / zoom, my = (e.clientY - r.top) / zoom; setMatDrag({ offX: mx - ox, offY: my - oy }); if (!matXY) setMatXY({ x: ox, y: oy }); }}
                      ...
                      <button onClick={e => { e.stopPropagation(); setMatCols(c => Math.max(2, c - 1)); }} style={{ width: 30, height: 30, borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.15)", cursor: "pointer", color: "#fff", fontSize: 18, fontWeight: 900 }}>−</button>
                      <span style={{ flex: 1, textAlign: "center", fontSize: 14, fontWeight: 900, color: "#fff", letterSpacing: 1.5 }}>{t("placeTable")}</span>
                      <button onClick={e => { e.stopPropagation(); setMatCols(c => Math.min(4, c + 1)); }} style={{ width: 30, height: 30, borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.15)", cursor: "pointer", color: "#fff", fontSize: 18, fontWeight: 900 }}>+</button>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: `repeat(${visCols.length},minmax(96px,1fr))` }}>
                      ...
                      {visCols.map(col => <button key={col.t + "+"} onClick={() => setMatCounts(p => ({ ...p, [col.t]: p[col.t] + 1 }))} style={{ padding: "10px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,.04)", borderRight: "1px solid rgba(60,50,30,.03)", background: col.c + "08", cursor: "pointer", fontSize: 22, fontWeight: 900, color: col.c, fontFamily: "inherit", textAlign: "center" }}>+</button>)}
                      ...
                      {visCols.map(col => { const v = matCounts[col.t]; return <button key={col.t + "-"} onClick={() => setMatCounts(p => ({ ...p, [col.t]: Math.max(0, p[col.t] - 1) }))} disabled={v < 1} style={{ padding: "10px 0", border: "none", borderTop: "1px solid rgba(0,0,0,.04)", borderBottom: "1px solid rgba(0,0,0,.06)", borderRight: "1px solid rgba(60,50,30,.03)", background: v > 0 ? col.c + "08" : "transparent", cursor: v > 0 ? "pointer" : "default", fontSize: 22, fontWeight: 900, color: v > 0 ? col.c : "#ddd", fontFamily: "inherit", textAlign: "center" }}>−</button>; })}
