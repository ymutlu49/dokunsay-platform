...
// A11y panelindeki ses aç/kapa anahtarını YOK SAYIYORDU — sesi kapatmak bu uygulamada
// hiçbir işe yaramıyordu. Ayrıca hız 0.8 idi, standart 0.85.
//
// DİL NOTU: Buradaki 'tr' KASITLI. Arayüz beş dilli ama SESLENDİRİLEN İÇERİK
// (timeTr okunuşu, "Harika!", "Doğru!", "Tekrar dene.") sabit Türkçe yazılmış.
// Dili `lang`'e bağlamak durumu KÖTÜLEŞTİRİRDİ: İngilizce sesle Türkçe metin okunurdu.
// Asıl eksik içeriğin yerelleştirilmesi — araç bazlı denetimde ele alınacak.
// Seslendirme dili artik PARAMETRE. Geri bildirim cumleleri yerellestirildi (asagi bak),
// ama timeTr() okunusu hâlâ Turkce uretiyor -> orada bilerek 'tr' gecilir. Yanlis dilde
// ses, sessizlikten daha kotudur: Ingilizce sesle Turkce metin okumak anlasilmaz olur.
function speak(t, lg){ platformSpeak(t, lg || 'tr'); }

// Oyun geri bildirimleri — onceden sabit Turkceydi ve her dilde Turkce okunuyordu.
var FB={
  great:{tr:"Harika!",ku:"Cana!",en:"Great!",ar:"رائع!",fa:"عالی!"},
  correct:{tr:"Doğru!",ku:"Rast e!",en:"Correct!",ar:"صحيح!",fa:"درست!"},
  wrong:{tr:"Yanlış.",ku:"Şaş e.",en:"Not quite.",ar:"غير صحيح.",fa:"درست نیست."},
  retry:{tr:"Tekrar dene.",ku:"Careke din biceribîne.",en:"Try again.",ar:"حاول مرّة أخرى.",fa:"دوباره امتحان کن."},
};
var fb=function(key,lg){return (FB[key]||{})[lg]||FB[key].tr;};
...
export default function App({ lang = 'tr' }){
  ...
  function checkGame(){if(!game)return;var ok=hour===game.th&&min===game.tm;if(ok)speak(fb("great",lang),lang);else speak(fb("retry",lang),lang);setGame(Object.assign({},game,{feedback:ok?"correct":"wrong",score:game.score+(ok?1:0),total:game.total+1}));if(ok)setTimeout(nextGameQ,1500);}
  ...
  function pickElapsed(a){if(!game||game.feedback)return;var ok=a===game.answer;if(ok)speak(fb("correct",lang),lang);else speak(fb("retry",lang),lang);setGame(Object.assign({},game,{feedback:ok?"correct":"wrong",score:game.score+(ok?1:0),total:game.total+1}));if(ok)setTimeout(startElapsedGame,1500);}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",fontFamily:"'Nunito','Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {/* SIDEBAR */}
        <div style={{width:col?52:220,minWidth:col?52:220,background:"linear-gradient(180deg,"+P.side+",#f3ede0)",borderRight:"1px solid "+P.sideB,display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden"}}>
          {!col?(<div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
            ...
            <div style={{padding:"4px 10px",background:"rgba(0,0,0,.02)"}}>
              <AppTabs active={sTab} onChange={setSTab} tabs={[
                {id:"mat",icon:"📦",label:({tr:"Materyaller",ku:"Materyal",en:"Materials",ar:"الأدوات",fa:"ابزارها"})[lang]||"Materyaller"},
                {id:"act",icon:"📋",label:({tr:"Etkinlikler",ku:"Çalakî",en:"Activities",ar:"الأنشطة",fa:"فعالیت‌ها"})[lang]||"Etkinlikler"},
                {id:"game",icon:"🎮",label:({tr:"Oyunlar",ku:"Lîstik",en:"Games",ar:"الألعاب",fa:"بازی‌ها"})[lang]||"Oyunlar"},
                {id:"feat",icon:"⚙️",label:({tr:"Ayarlar",ku:"Sazkirin",en:"Settings",ar:"الإعدادات",fa:"تنظیمات"})[lang]||"Ayarlar"},
              ]}/>
            </div>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"10px 12px",scrollbarWidth:"none",msOverflowStyle:"none",WebkitOverflowScrolling:"touch"}}>
              <div style={{background:game?"rgba(34,197,94,.04)":"#fff",borderRadius:14,padding:"12px",marginBottom:8,border:game?"1.5px solid rgba(34,197,94,.2)":"1px solid rgba(0,0,0,.05)"}}>
                {!game?(<div style={{display:"flex",flexDirection:"column",gap:3}}>
                  ...
                  <button onClick={function(){startMatchGame("reverse");}} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid rgba(34,197,94,.15)",background:"rgba(34,197,94,.03)",cursor:"pointer",fontSize:10,fontWeight:700,color:P.accentD,fontFamily:"inherit",textAlign:"left"}}>{"Dijital → Analog"}</button>
                  <div style={{height:1,background:"rgba(0,0,0,.06)",margin:"6px 0"}}/>
                  ...
                  <button onClick={startRoutineGame} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid rgba(34,197,94,.15)",background:"rgba(34,197,94,.03)",cursor:"pointer",fontSize:10,fontWeight:700,color:P.accentD,fontFamily:"inherit",textAlign:"left"}}>{"Günlük Rutin Bilgisi"}</button>
                </div>):(<div>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"10px 12px",scrollbarWidth:"none",msOverflowStyle:"none",WebkitOverflowScrolling:"touch"}}>
              ...
              {["keşif","kavram","karşılaştır","yanılgı"].map(function(cat){var acts=ACTS.filter(function(a){return a.cat===cat;});if(!acts.length)return null;return <div key={cat}><div style={{fontSize:8,fontWeight:800,textTransform:"uppercase",letterSpacing:1.5,color:cat==="yanılgı"?P.red:"#999",margin:"8px 0 4px"}}>{({keşif:{tr:"Keşif",ku:"Vekolîn",en:"Explore",ar:"استكشاف",fa:"کاوش"},kavram:{tr:"Kavram",ku:"Têgeh",en:"Concept",ar:"مفهوم",fa:"مفهوم"},"karşılaştır":{tr:"Karşılaştırma",ku:"Berhevdan",en:"Compare",ar:"مقارنة",fa:"مقایسه"},"yanılgı":{tr:"Yanılgı",ku:"Şaşitî",en:"Misconception",ar:"مفهوم خاطئ",fa:"کج‌فهمی"}}[cat]||{})[lang]||cat}</div>{acts.map(function(tp,i){var isA=aTpl&&aTpl.n===tp.n;return <button key={i} onClick={function(){loadA(tp);}} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 8px",width:"100%",background:isA?P.accentLight:P.card,border:isA?"2px solid "+P.accent:"1px solid "+P.sideB,borderRadius:8,cursor:"pointer",fontFamily:"inherit",textAlign:"left",color:"#3d3520",marginBottom:3,fontSize:10,fontWeight:isA?900:600}}><span style={{fontSize:14}}>{tp.i}</span><span style={{flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{actName(tp,lang)}</span></button>;})}</div>;})}
            </div>):null}
        ...
        <div ref={cvRef} style={Object.assign({flex:1,position:"relative",overflow:"auto"},cvBg)}>
          {/* Floating dijital saat (eski iç-header yerine) */}
          <div style={{position:"absolute",top:8,right:8,zIndex:7,padding:"4px 12px",background:"rgba(26,26,26,.92)",backdropFilter:"blur(8px)",borderRadius:8,boxShadow:"0 2px 8px rgba(0,0,0,.18)",border:"1px solid rgba(255,255,255,.1)"}}>
            <span style={{fontSize:14,fontWeight:900,color:"#e2b93b",fontFamily:"monospace"}}>{timeDig(hour,min)}</span>
          </div>
          {/* Araç çubuğu */}
          <div style={{position:"absolute",top:10,left:"50%",transform:"translateX(-50%)",zIndex:30,display:"flex",gap:3,background:"rgba(255,255,255,.95)",backdropFilter:"blur(12px)",borderRadius:14,padding:"5px 8px",boxShadow:"0 4px 20px rgba(0,0,0,.08)",alignItems:"center"}}>
            {[["select","🖱️","Seç"],["pen","✏️","Çiz"],["highlighter","🖍️","Fosforlu"],["eraser","🧹","Sil"],["text","T","Yaz"]].map(function(t){return <IconButton key={t[0]} icon={t[1]} active={tool===t[0]} onClick={function(){setTool(t[0]);}} title={t[2]}/>;})}
            ...
            <IconButton icon="↩" title="Geri al" onClick={undo} disabled={!strokes.length}/>
            <IconButton icon="↪" title="İleri al" onClick={redo} disabled={!undone.length}/>
            <IconButton icon="🗑" title="Çizimi temizle" danger onClick={function(){setStrokes([]);setTexts([]);}}/>
          </div>
          ...
          {sDr&&dropH?<div style={{position:"absolute",inset:0,zIndex:0,border:"3px dashed rgba(34,197,94,.4)",borderRadius:4,pointerEvents:"none"}}><div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"rgba(34,197,94,.1)",padding:"8px 20px",borderRadius:12}}><span style={{fontSize:13,fontWeight:800,color:"rgba(34,197,94,.6)"}}>{"📥 Buraya bırak"}</span></div></div>:null}
      ...
      <div style={{height:42,minHeight:42,background:P.side,borderTop:"1px solid "+P.sideB,display:"flex",alignItems:"center",padding:"0 20px",gap:10}}>
        ...
        <div style={{flex:1}}/>
        {/* Zoom kontrolleri */}
        <div style={{display:"flex",gap:2,alignItems:"center",background:"#fff",borderRadius:7,padding:"2px 6px",border:"1.5px solid rgba(0,0,0,.08)"}}>
          <button onClick={function(){setZoom(function(z){return Math.max(0.5,+(z-0.1).toFixed(1));});}} aria-label="Uzaklaştır" style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:14,fontWeight:900,width:22,height:22}}>{"−"}</button>
          <span style={{fontSize:10,fontWeight:800,color:"#3d3520",minWidth:32,textAlign:"center"}}>{Math.round(zoom*100)+"%"}</span>
          <button onClick={function(){setZoom(function(z){return Math.min(2,+(z+0.1).toFixed(1));});}} aria-label="Yakınlaştır" style={{background:"none",border:"none",color:"#475569",cursor:"pointer",fontSize:14,fontWeight:900,width:22,height:22}}>{"+"}</button>
        </div>
        <div style={{width:1,height:22,background:"rgba(0,0,0,.06)"}}/>
      ...
      {tcO?<div onClick={function(){setTcO(false);}} style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center"}}><div onClick={function(e){e.stopPropagation();}} style={{background:"#fff",borderRadius:24,padding:"28px 32px",maxWidth:580,width:"92%",maxHeight:"90vh",overflowY:"auto",animation:"popIn .3s"}}>
        ...
        <div style={{background:"rgba(139,92,246,.04)",borderRadius:14,padding:"14px",marginBottom:14,border:"1.5px solid rgba(139,92,246,.12)"}}>
          ...
          {LESSONS.map(function(ls,li){var lActs=ls.acts||[];var lDone=lActs.filter(function(ai){return comp[ACTS[ai]&&ACTS[ai].n];}).length;var lTotal=lActs.length;return <div key={li} style={{padding:"8px 10px",marginBottom:4,borderRadius:8,background:"#fff",border:"1px solid rgba(0,0,0,.05)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,fontWeight:800,color:"#3d3520"}}>{actName(ls,lang)}</span><span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:4,background:lDone===lTotal&&lTotal>0?"rgba(34,197,94,.1)":"rgba(0,0,0,.04)",color:lDone===lTotal&&lTotal>0?P.green:"#aaa"}}>{lDone+"/"+lTotal}</span></div>
            <div style={{fontSize:9,color:"#888",marginTop:2}}>{actDesc(ls,lang)}</div>
          </div>;})}
        </div>
        {/* Oyun skorları */}
        {game?<div style={{background:"rgba(34,197,94,.04)",borderRadius:14,padding:"14px",marginBottom:14,border:"1.5px solid rgba(34,197,94,.12)"}}>
