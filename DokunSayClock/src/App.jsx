...
export default function App(){
  ...
  function renderItem(it){var key="it"+it.id;var base={position:"absolute",left:it.x,top:it.y,zIndex:it.t==="handH"||it.t==="handM"?3:it.t==="digi"||it.t==="verb"?5:1,cursor:"grab",touchAction:"none"};function pd(e){if(dragH)return;startItemDrag(it.id,e);}
    ...
    if(it.t==="hnum"){var cr=Math.max(16,R*.105);return <div key={key} style={Object.assign({},base,{width:cr*2+4,height:cr*2+4})} onPointerDown={pd}><svg width={cr*2+4} height={cr*2+4} role="img" aria-label={"Saat sayısı "+it.v}><title>{"Saat sayısı "+it.v}</title><circle cx={cr+2} cy={cr+2} r={cr} fill={P.hourC} stroke={P.hourB} strokeWidth={2.5}/><text x={cr+2} y={cr+3} textAnchor="middle" dominantBaseline="middle" fontSize={Math.max(14,R*.095)} fontWeight={900} fill="#fff" data-numeric="true">{it.v}</text></svg></div>;}
    if(it.t==="mnum"){var mc=Math.max(12,R*.075);return <div key={key} style={Object.assign({},base,{width:mc*2+4,height:mc*2+4})} onPointerDown={pd}><svg width={mc*2+4} height={mc*2+4} role="img" aria-label={"Dakika "+it.v}><title>{"Dakika "+it.v}</title><circle cx={mc+2} cy={mc+2} r={mc} fill={P.minC} stroke={P.minB} strokeWidth={2}/><text x={mc+2} y={mc+3} textAnchor="middle" dominantBaseline="middle" fontSize={Math.max(9,R*.06)} fontWeight={800} fill="#fff" data-numeric="true">{it.v===60?"60":(it.v<10?"0"+it.v:it.v)}</text></svg></div>;}
    if(it.t==="h24"){var h24r=Math.max(14,R*.09);return <div key={key} style={Object.assign({},base,{width:h24r*2+4,height:h24r*2+4})} onPointerDown={pd}><svg width={h24r*2+4} height={h24r*2+4} role="img" aria-label={"24 saat "+it.v}><title>{"24 saat "+it.v}</title><circle cx={h24r+2} cy={h24r+2} r={h24r} fill="#8b5cf6" stroke="#6d28d9" strokeWidth={2}/><text x={h24r+2} y={h24r+3} textAnchor="middle" dominantBaseline="middle" fontSize={Math.max(10,R*.07)} fontWeight={800} fill="#fff" data-numeric="true">{it.v<10?"0"+it.v:it.v}</text></svg></div>;}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",fontFamily:"'Nunito','Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{height:42,minHeight:42,background:"linear-gradient(135deg,#1a1a1a,#2d2520)",display:"flex",alignItems:"center",padding:"0 16px",gap:10,boxShadow:"0 4px 20px rgba(0,0,0,.2)"}}>
        <div style={{flex:1}}/>
        <div style={{padding:"4px 14px",background:"rgba(255,255,255,.06)",borderRadius:10}}><span data-numeric="true" style={{fontSize:18,fontWeight:900,color:"#e2b93b",fontFamily:"monospace"}}>{timeDig(hour,min)}</span></div>
        <div role="group" aria-label="Yakınlaştır / Uzaklaştır" style={{display:"flex",gap:3,alignItems:"center",background:"rgba(255,255,255,.06)",borderRadius:8,padding:"4px 8px"}}><button onClick={function(){setZoom(function(z){return Math.max(0.5,+(z-0.1).toFixed(1));});}} aria-label="Uzaklaştır" title="Uzaklaştır" style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",cursor:"pointer",fontSize:14,fontWeight:900}}><span aria-hidden="true">{"−"}</span></button><span style={{fontSize:10,color:"rgba(255,255,255,.35)",minWidth:36,textAlign:"center"}} aria-live="polite" data-numeric="true">{Math.round(zoom*100)+"%"}</span><button onClick={function(){setZoom(function(z){return Math.min(2,+(z+0.1).toFixed(1));});}} aria-label="Yakınlaştır" title="Yakınlaştır" style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",cursor:"pointer",fontSize:14,fontWeight:900}}><span aria-hidden="true">{"+"}</span></button></div>
      </div>
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{width:col?52:260,minWidth:col?52:260,background:"linear-gradient(180deg,"+P.side+",#f3ede0)",borderRight:"1px solid "+P.sideB,display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden"}}>
          {!col?(<div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
            <div style={{padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"flex-end",gap:10,borderBottom:"1px solid rgba(0,0,0,.05)"}}><button onClick={function(){setCol(true);}} aria-label="Paneli daralt" title="Paneli daralt" aria-expanded="true" style={{background:"rgba(0,0,0,.04)",border:"none",cursor:"pointer",fontSize:14,color:"#bbb",width:28,height:28,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}><span aria-hidden="true">{"◀"}</span></button></div>
            <div role="tablist" aria-label="Materyal / Etkinlik / Oyun / Ayarlar" style={{display:"flex",padding:"6px 12px",gap:3,background:"rgba(0,0,0,.02)"}}>
              {[["📦","mat","Materyal"],["📋","act","Etkinlik"],["🎮","game","Oyun"],["⚙️","feat","Ayarlar"]].map(function(t){return <button key={t[1]} onClick={function(){setSTab(t[1]);}} role="tab" aria-selected={sTab===t[1]} aria-label={t[2]} title={t[2]} style={{flex:1,padding:"7px 0",border:"none",borderRadius:8,background:sTab===t[1]?"#fff":"transparent",cursor:"pointer",fontSize:13,fontWeight:800,color:sTab===t[1]?"#3d3520":"#aaa",fontFamily:"inherit",boxShadow:sTab===t[1]?"0 1px 4px rgba(0,0,0,.06)":"none"}}><span aria-hidden="true">{t[0]}</span></button>;})}
            </div>
          ...
          </div>):(<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"12px 0"}}>{[["📦","mat","Materyal menüsü"],["📋","act","Etkinlik menüsü"],["🎮","game","Oyun menüsü"],["⚙️","feat","Ayarlar menüsü"]].map(function(ti,ii){return <button key={ii} onClick={function(){setCol(false);setSTab(ti[1]);}} aria-label={ti[2]} title={ti[2]} style={{padding:"8px 12px",borderRadius:8,border:"1px solid "+P.sideB,background:"#fff",cursor:"pointer",fontSize:18}}><span aria-hidden="true">{ti[0]}</span></button>;})}</div>)}
        </div>
        {/* KANVAS */}
        <div ref={cvRef} style={Object.assign({flex:1,position:"relative",overflow:"auto"},cvBg)}>
          ...
          <div style={{position:"absolute",top:10,left:"50%",transform:"translateX(-50%)",zIndex:30,display:"flex",gap:3,background:"rgba(255,255,255,.95)",backdropFilter:"blur(12px)",borderRadius:14,padding:"5px 8px",boxShadow:"0 4px 20px rgba(0,0,0,.08)",alignItems:"center"}}>
            {[["select","🖱️","Seç"],["pen","✏️","Kalem"],["highlighter","🖍️","Vurgulayıcı"],["eraser","🧹","Silgi"],["text","T","Metin"]].map(function(t){return <button key={t[0]} onClick={function(){setTool(t[0]);}} aria-label={t[2]} aria-pressed={tool===t[0]} title={t[2]} style={{width:34,height:34,borderRadius:8,border:tool===t[0]?"2px solid "+P.accent:"2px solid transparent",background:tool===t[0]?"rgba(245,158,11,.1)":"transparent",cursor:"pointer",fontSize:t[0]==="text"?13:15,fontWeight:t[0]==="text"?900:400,color:"#3d3520",display:"flex",alignItems:"center",justifyContent:"center"}}><span aria-hidden="true">{t[1]}</span></button>;})}
            ...
            <button onClick={undo} disabled={!strokes.length} aria-label="Geri al" title="Geri al" style={{width:34,height:34,borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:15,color:strokes.length?"#666":"#ddd"}}><span aria-hidden="true">{"↩"}</span></button>
            <button onClick={redo} disabled={!undone.length} aria-label="İleri al" title="İleri al" style={{width:34,height:34,borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:15,color:undone.length?"#666":"#ddd"}}><span aria-hidden="true">{"↪"}</span></button>
            <button onClick={function(){setStrokes([]);setTexts([]);}} aria-label="Çizimleri temizle" title="Çizimleri temizle" style={{width:34,height:34,borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:15,color:"#999"}}><span aria-hidden="true">{"🗑"}</span></button>
          </div>
      ...
      <div style={{height:42,minHeight:42,background:P.side,borderTop:"1px solid "+P.sideB,display:"flex",alignItems:"center",padding:"0 20px",gap:10}}>
        {[["Düz","plain"],["Kareli","grid"],["Noktalı","dot"]].map(function(b){return <button key={b[1]} onClick={function(){setBgT(b[1]);}} aria-label={b[0]+" arka plan"} aria-pressed={bgT===b[1]} title={b[0]+" arka plan"} style={{padding:"4px 12px",borderRadius:8,border:bgT===b[1]?"2px solid "+P.accent:"1.5px solid rgba(0,0,0,.06)",background:bgT===b[1]?"rgba(245,158,11,.08)":"#fff",cursor:"pointer",fontSize:10,fontWeight:700,color:bgT===b[1]?"#92400e":"#999"}}>{b[0]}</button>;})}
        <div style={{width:1,height:22,background:"rgba(0,0,0,.06)"}}/>
        {[["Bej",P.bg],["Beyaz","#fff"],["Mavi","#e8f0fe"],["Gri","#f0f0f0"],["Koyu","#2a2a2a"]].map(function(c){return <button key={c[1]} onClick={function(){setBgC(c[1]);}} aria-label={c[0]+" renk"} aria-pressed={bgC===c[1]} title={c[0]+" renk"} style={{width:22,height:22,borderRadius:"50%",border:bgC===c[1]?"3px solid "+P.accent:"2px solid rgba(0,0,0,.08)",background:c[1],cursor:"pointer"}}/>;})}
        <div style={{flex:1}}/>
        <button onClick={function(){setHlO(true);}} aria-label="Yardım" title="Yardım" style={{width:28,height:28,borderRadius:8,border:"1.5px solid rgba(0,0,0,.08)",background:"#fff",cursor:"pointer",fontSize:12,fontWeight:800,color:"#999",display:"flex",alignItems:"center",justifyContent:"center"}}><span aria-hidden="true">{"?"}</span></button>
        <button onClick={function(){setAbO(true);}} aria-label="Hakkında" title="Hakkında" style={{width:28,height:28,borderRadius:8,border:"1.5px solid rgba(0,0,0,.08)",background:"#fff",cursor:"pointer",fontSize:12,color:"#999",display:"flex",alignItems:"center",justifyContent:"center"}}><span aria-hidden="true">{"ℹ"}</span></button>
        <button onClick={function(){setTcO(true);}} aria-label="Öğretmen paneli" title="Öğretmen paneli" style={{width:28,height:28,borderRadius:8,border:"1.5px solid rgba(0,0,0,.08)",background:"#fff",cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}><span aria-hidden="true">{"👨‍🏫"}</span></button>
      </div>
      {/* Ghosts */}
