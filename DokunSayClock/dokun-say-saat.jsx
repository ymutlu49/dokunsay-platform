      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{width:col?52:260,minWidth:col?52:260,background:"linear-gradient(180deg,"+P.side+",#f3ede0)",borderRight:"1px solid "+P.sideB,display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden"}}>
          {!col?(<div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
            ...
            {sTab==="mat"?(<div style={{flex:1,overflowY:"auto",padding:"10px 12px"}}>
              {/* Kadranlar */}
              ...
              {/* Kollar */}
              <div style={{background:"#fff",borderRadius:14,padding:"12px",marginBottom:10,border:"1px solid rgba(0,0,0,.05)"}}>
                <div style={{fontSize:10,fontWeight:800,textTransform:"uppercase",letterSpacing:1.5,color:"#555",marginBottom:10}}>{"Kollar & Gösterimler"}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  <div style={{background:"rgba(17,17,17,.03)",borderRadius:10,padding:"10px 6px",cursor:"grab",touchAction:"none",textAlign:"center",border:"1.5px solid rgba(0,0,0,.08)"}} onPointerDown={function(e){stSD("handH",0,e);}}><div style={{fontSize:22,marginBottom:2}}>{"🕐"}</div><div style={{fontSize:9,fontWeight:800,color:"#444"}}>{"Saat Göstergesi"}</div></div>
                  <div style={{background:"rgba(30,64,175,.03)",borderRadius:10,padding:"10px 6px",cursor:"grab",touchAction:"none",textAlign:"center",border:"1.5px solid rgba(30,64,175,.12)"}} onPointerDown={function(e){stSD("handM",0,e);}}><div style={{fontSize:22,marginBottom:2}}>{"🕑"}</div><div style={{fontSize:9,fontWeight:800,color:"#1e40af"}}>{"Dakika Göstergesi"}</div></div>
                  <div style={{background:"#0f0f0f",borderRadius:10,padding:"10px 6px",cursor:"grab",touchAction:"none",textAlign:"center",border:"1.5px solid #333"}} onPointerDown={function(e){stSD("digi",0,e);}}><div style={{fontSize:11,fontWeight:900,color:"#e2b93b",fontFamily:"monospace",margin:"4px 0"}}>{"12:00"}</div><div style={{fontSize:9,fontWeight:800,color:"#888"}}>{"Dijital Saat"}</div></div>
                  <div style={{background:"rgba(139,92,246,.03)",borderRadius:10,padding:"10px 6px",cursor:"grab",touchAction:"none",textAlign:"center",border:"1.5px solid rgba(139,92,246,.1)"}} onPointerDown={function(e){stSD("verb",0,e);}}><div style={{fontSize:16,margin:"2px 0"}}>{"🔊"}</div><div style={{fontSize:9,fontWeight:800,color:"#6d28d9"}}>{"Sözel Okunuş"}</div></div>
                </div>
              </div>
              <div style={{display:"flex",gap:6,marginBottom:10}}><button onClick={placeAll} style={{flex:1,padding:"10px 0",borderRadius:10,border:"none",background:"linear-gradient(135deg,"+P.accent+","+P.accentD+")",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{"⚡ Hepsini Yerleştir"}</button><button onClick={clearAll} style={{flex:1,padding:"10px 0",borderRadius:10,border:"1.5px solid "+P.sideB,background:"#fff",color:"#888",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>{"🗑 Temizle"}</button></div>
            </div>):sTab==="game"?(
            /* ★ OYUNLAR SEKMESİ */
